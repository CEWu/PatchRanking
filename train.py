...
def reset_cfg(cfg, args):
    ...
    if args.split:
        cfg.TRAINER.COOP.SPLIT = args.split

    if args.rank_mode:
        cfg.TRAINER.COOP.RANK_MODE = args.rank_mode
...
def extend_cfg(cfg):
    ...
    cfg.TRAINER.COOP.IS_PRUNE = False
    cfg.TRAINER.COOP.TOKENS_SCORE = None
    cfg.TRAINER.COOP.BLOCK_SIZE = 0
    cfg.TRAINER.COOP.SPLIT = 'train'
    cfg.TRAINER.COOP.RANK_MODE = 'similarity'
...
def main(args):
    ...
    #     rank_dir = os.path.join(args.output_dir, 'tokens_rank')
    #     if not os.path.exists(rank_dir):
    #         os.makedirs(rank_dir)
    #     tokens_rank_path = os.path.join(rank_dir, "{}_tokens_rank.json".format(dataset_name))
    #     tokens_rank_dict = {i: 0.0 for i in range(1, 197)}
    #     for token_id in tokens_rank_dict.keys():        
    #         trainer.test_rank(token_id=token_id, tokens_rank=tokens_rank_dict)
    #     with open(tokens_rank_path, "w") as outfile:
    #         json.dump(tokens_rank_dict, outfile)
    #     return
    split = args.split
    if args.eval_rank:
        trainer.load_model(args.model_dir, epoch=args.load_epoch)
        trainer.test_rank(split=split)
        return

    if args.eval_prune:
        trainer.load_model(args.model_dir, epoch=args.load_epoch)
        trainer.test_prune(split=split)
        return

    if args.eval_mask:
        trainer.load_model(args.model_dir, epoch=args.load_epoch)
        trainer.test_mask(split=split)
        return

    if args.train_rank:
        trainer.train()
        # trainer.load_model(args.model_dir, epoch=args.load_epoch)
        # trainer.train_rank(split=split)
        return
...
if __name__ == "__main__":
    ...
    parser.add_argument("--eval-rank", action="store_true", help="evaluation of rank")
    parser.add_argument("--eval-prune", action="store_true", help="evaluation of rank")
    parser.add_argument("--eval-mask", action="store_true", help="evaluation of rank")
    parser.add_argument("--train-rank", action="store_true", help="training of rank")
    parser.add_argument(
        "--model-dir",
        type=str,
        default="",
    ...
    parser.add_argument(
        ...
        type=str,
        default="similarity",
        choices=['label', 'max', 'similarity'],
        help="The mode of ranking to be used. Options are: 'label', 'max', or 'similarity'."
    )
